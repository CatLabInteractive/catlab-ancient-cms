...
class Core_Tools
{
	...
	*/
	...
	public static function fixTinyMCEInput ($content)
	{
		// Remove all absolute url's
		$content = str_replace (CMS_FULL_URL, '', $content);
		return $content;
	}
	...
	{	
		...
		(	"/\[quote:(.*)]/s", 
			$p_close."<h3>".'Quote'." \\1:</h3><blockquote>", 
			$convert
		);

		$convert = str_replace("[quote]", $p_close."<blockquote>", $convert);
		...
		if (substr ($convert, 0, strlen ($p_close)) == $p_close)
		{
			$convert = substr ($convert, strlen ($p_close));
		}
	...
	public static function output_form ($text, $maxLength = 0)
	{
		if ($maxLength > 0)
		{
			return substr (htmlentities (stripslashes ($text) , ENT_QUOTES, 'UTF-8'), 0, $maxLength);
		}
		else
		{
	...
	public static function output_input ($text, $maxLength = 0)
	{
		return self::output_form (str_replace ("\n", '', $text), $maxLength);
	}
	
	public static function output_html ($text, $forEditing = false)
	{
		$text = stripslashes ($text);
		
		// Replace all relative url's with absolute one's (for the predefined constants)
		$text = preg_replace ('/src="(?!http)/', 'src="'.CMS_FULL_URL, $text);
		$text = preg_replace ('/href="(?!http)/', 'href="'.CMS_FULL_URL, $text);
		$text = preg_replace ('/about="(?!http)/', 'about="'.CMS_FULL_URL, $text);
		
		$rand = rand (0, 250);
		
		if (!$forEditing)
		{
			// Create thumbnails from "about-ed" images
			$text = preg_replace 
			(
				'/<img([^>]+)about="([^">]+)"([^>]+)>/', 
				'<a href="$2" rel="lightbox['.$rand.']"><img $1 $3 /></a>', 
				$text
			);
		}
	...
	public static function splitInPages ($page, $total, $current = 0, $perpage = 10, $action = '', $maxAantalSnelclicks = 10)
	{
		...
		$page->set ('pagelist_action', $action);
