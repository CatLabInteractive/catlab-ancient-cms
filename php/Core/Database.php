...
define ('MYSQL_TABLE_NOT_EXIST', 1146);
...
class Core_Database
{
	...
	public function connect ($server, $user, $password, $database)
	{
		...
		{
			die ('<p><strong>Could not connect to database.</strong><br />Database not found.</p>');
		}
	...
	private function query ($sql_query)
	{
		...
		{
			//echo $sql_query;
			throw new Core_DBError (mysql_error (), mysql_errno());
		}
	...
	{
		$query = $this->getDataFromQuery ($this->query ("SELECT FROM_UNIXTIME('{$timestamp}') AS datum"));
		return $query[0]['datum'];
	}
	
	public function toUnixtime ($date)
	{
		$query = $this->getDataFromQuery ($this->query ("SELECT UNIX_TIMESTAMP('{$date}') AS datum"));
		return $query[0]['datum'];
	}
}
