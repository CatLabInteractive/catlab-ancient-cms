...
************************************************************************************************************/	
	...
	function JSDragDropTree()
	{
		...
		this.imageFolder = CMS.settings.path_static + 'images/';
		this.iconFolder = CMS.settings.path_icons;
		this.folderImage = 'folder_page.png';
	...
	JSDragDropTree.prototype = {
		...
		*/
		...
		createDropIndicator : function()
		{
			...
			this.dropTargetIndicator.id = 'dragDropIndicator';
			...
			if(lis.length>0){
				...
				while(li){
					if(li.id){
						...
						var numericID = li.id.substr (4);
						var numericParentID = li.parentNode.parentNode.id.substr (4);
						if(numericID!='0'){
		...
		initTree : function()
		{
			...
			for(var no=0;no<menuItems.length;no++){
				...
				{
					folderImg.src = this.iconFolder + menuItems[no].className + '.png';
				}
				else
				{
					folderImg.src = this.iconFolder + this.folderImage;
				}
